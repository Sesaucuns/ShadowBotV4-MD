        ...
        for (let name in global.plugins) {
            ...
            if (typeof plugin.all === 'function') {
                try {
                    ...
                    for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                        ...
                        if (data.exists)
                            m.reply(`*[ ⚠ COMANDOS CON FALLOS ⚠ ]*\n\n*❍ PLUGIN:* ${name}\n*❍ USUARIO:* ${m.sender}\n*❍ COMANDO:* ${m.text}\n\n*❍ ERROR:*\n\`\`\`${format(e)}\`\`\`\n\n⚠️ REPORTALO A MI CREADOR CON #REPORTE`.trim(), data.jid)
                    }
                }
            }
            if (!opts['restrict'])
	        ...
	        let hl = _prefix 
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `*⚠️ NO TIENES DIAMANTES PUEDES COMPRAR CON EL COMANDO ${usedPrefix}buy <cantidad>*`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `*⚠️ NECESITAS EL NIVEL ${plugin.level} PARA USAR ESTE COMANDO. TU NIVEL ES ${_user.level}*`, m)
                    ...
                    if (e) {
                        ...
                        if (e.name)
                            for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists)
                                    m.reply(`*[ ⚠ ️COMANDO CON FALLOS ⚠ ️]*\n\n*❍ PLUGIN:* ${m.plugin}\n*❍ USUARIO:* ${m.sender}\n*❍ COMANDO:* ${usedPrefix}${command} ${args.join(' ')}\n\n\`\`\`${text}\`\`\`\n\n*⚠️ REPÓRTALO A MI CREADOR CON #reporte*`.trim(), data.jid)
                            }
                        m.reply(text)
                    }
                } finally {
...
global.dfail = (type, m, conn) => {
    ...
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '[ ⚠ ] *AVISO*', body: 'The Shadow Brokers - Bot - MD', thumbnail: imagen1, sourceUrl: 'https://github.com/DIEGO-OFC/ShadowBotV4-MD' }}}}, aa)
