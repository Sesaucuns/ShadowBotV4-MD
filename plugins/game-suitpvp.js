...
if (Object.values(conn.suit).find(room => room.id.startsWith('suit') && [room.p, room.p2].includes(m.sender))) throw '*⚠️ TERMINA TU PARTIDA ANTES DE INICIAR OTRA*'
let textquien = `*⚠️ A QUIEN QUIERE RETAR? ETIQUETA A LA PERSONA*\n\n*—◉ EJEMPLO:*\n${usedPrefix}suit @${global.suittag}`
if (!m.mentionedJid[0]) return m.reply(textquien, m.chat, { mentions: conn.parseMention(textquien)})
if (Object.values(conn.suit).find(room => room.id.startsWith('suit') && [room.p, room.p2].includes(m.mentionedJid[0]))) throw `*[❗] LA PERSONA A LA QUE QUIERE DESAFIAR AUN ESTA JUGANDO OTRA PARTIDA, ESPERA A QUE TERMINE DE JUGAR`
...
let footer = `◉ ESCRIBE "aceptar" PARA COMENZAR\n◉ ESCRIBE "rechazar" PARA RECHAZAR`
